    ...
    return (
        <>
            <Flex flexDirection={"column"} alignItems={"center"} justifyContent={"center"} mt={20} width="100%">
                ...
                <Flex width="90%" justifyContent={"space-evenly"} mt="2rem" wrap={"wrap"} gap="12px">
                    {WhyChooseUsData.map((item) => (
                        <Flex flexDirection={"column"} alignItems={"center"} width={{ base: "60%", md: "27%", lg: "23%" }} key={item.id} pt={{ base: "10px", lg: `${item.id == 2 | item.id == 4 ? "55px" : "0px"}` }} pb={"2rem"} >
                            ...
                            <Flex width="100%" fontSize={"1rem"} fontWeight={"500"} justifyContent={{ base: "center", lg: "left" }} mt="0.5rem">{item.title}</Flex>
